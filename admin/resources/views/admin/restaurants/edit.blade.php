...
@section('title', 'Restoran Düzenle - ' . $restaurant['name'])
...
<div class="min-h-screen bg-gray-50">
    ...
    <header class="bg-gradient-to-r from-gray-800 to-gray-900 shadow-lg">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                ...
                <h1 class="text-xl font-bold text-white">Restoran Düzenle</h1>
                <div class="flex items-center space-x-2">
                    ...
                    <span class="text-gray-400">({{ $reviewCount }} değerlendirme)</span>
                </div>
            </div>
        </div>
    </header>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form action="{{ route('admin.restaurants.update', $restaurant['id']) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="username" id="username" value="{{ old('username', $restaurant['username'] ?? '') }}" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-transparent transition duration-200">
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">Boş bırakırsanız mevcut şifre korunur</p>
                    </div>

                    <div class="md:col-span-2">
                        <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Açıklama</label>
                        <textarea name="description" id="description" rows="3"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-transparent transition duration-200">{{ old('description', $restaurant['description']) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $restaurant['id'] }}" disabled
                            class="w-full px-4 py-3 border border-gray-200 bg-gray-50 rounded-lg text-gray-500">
                    </div>
                </div>
            </div>
                        ...
                        <div class="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div class="flex items-center space-x-2 flex-1">
                                <input type="time" name="working_hours[{{ $index }}][open]" 
                                    value="{{ $dayHours['open'] ?? '' }}"
                                    class="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-transparent transition duration-200 working-hours-time"
                                    data-day="{{ $index }}">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            @foreach($menuUrls as $index => $menuUrl)
                                <div class="relative group">
                                    ...
                                    <button type="button" onclick="removeMenu('{{ $restaurant['id'] }}', '{{ $menuUrl }}')"
...
<script>
    ...
    function removeMenu(restaurantId, menuUrl) {
        if (confirm('Bu menüyü silmek istediğinizden emin misiniz?')) {
            fetch(`/admin/restaurants/${restaurantId}/remove-menu`, {
                method: 'POST',
                headers: {
