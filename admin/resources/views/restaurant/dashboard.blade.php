                        ...
                        @foreach(['Pazartesi', 'Salı', 'Çarşamba', 'Perşembe', 'Cuma', 'Cumartesi', 'Pazar'] as $day)
                            <div class="flex justify-between items-center py-2 border-b border-gray-100 last:border-0">
                                ...
                                @if(isset($workingHours[$day]))
                                    @if($workingHours[$day]['closed'] ?? false)
                                        <span class="text-red-500 font-medium">Kapalı</span>
                                    @elseif(($workingHours[$day]['open'] ?? '') && ($workingHours[$day]['close'] ?? ''))
                                        <span class="text-gray-900">{{ $workingHours[$day]['open'] }} - {{ $workingHours[$day]['close'] }}</span>
                                    @else
